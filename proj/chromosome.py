...
class Chromosome:
    ...
    def repair_module(self):
        ...
        for node in overflow_nodes:
            for cluster in self.genes:
                if (
                    sum(airport.flow for airport in cluster[1:]) + node.flow
                    <= cluster[0].capacity
                ):
                    cluster.append(node)
        ...
        while overflow_nodes:
            ...
            while (
                overflow_nodes
                and (
                    sum(airport.flow for airport in cluster[1:])
                    + overflow_nodes[0].flow
                )
                <= hub_capacity
            ):
...
def generate_chromosome(num_hub, airports: List[Airport]) -> Chromosome:
    ...
    for location in locations:
        hub_index = (
            hubs.index(location) if location in hubs else random.randint(0, num_hub - 1)
        )
